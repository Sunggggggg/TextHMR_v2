...
class PW3D(torch.utils.data.Dataset):
    ...
    def load_data(self):
        ...
        for aid in db.anns.keys():  # 전체 이미지에 대한 idx : aid (0~22,734)
            aid = int(aid)
            ann = db.anns[aid]
            image_id = ann['image_id'] 
            img = db.loadImgs(image_id)[0]
            img_width, img_height = img['width'], img['height'] # [1080, 1920]
            sequence_name = img['sequence']                     # courtyard_rangeOfMotions_00
            img_name = img['file_name']                         # image_00000.jpg

            smpl_param = ann['smpl_param']
            pose_param = np.array(smpl_param['pose'], dtype=np.float32)     # 72
            shape_param = np.array(smpl_param['shape'], dtype=np.float32)   # 10
            ...
            joint_cam_coco = np.array(coco_cam_joints[seq_idx][img_idx][person_idx], dtype=np.float32)          # [19, 3]
            gt_joint_img_coco = np.array(gt_coco_img_joints[seq_idx][img_idx][person_idx], dtype=np.float32)    # [19, 3]
            joint_cam_h36m = np.array(h36m_cam_joints[seq_idx][img_idx][person_idx], dtype=np.float32)          # [17, 3]
